...
@export var score_value: int = 0 setget set_score
@export var distance: int = 0 setget set_distance
...
func _update_sprites(animate=false):
    ...
    for c in sprites.get_children():
        c.position = Vector2()
    # set layout
    var first_spr: Sprite2D = null
    var last_spr: Sprite2D = null
    var idx = 0
    for digit in str(score_value):
        var num_spr: Sprite2D = sprites.get_child(idx)
...
func _center_sprites(first_spr: Sprite2D, last_spr: Sprite2D):
    var width = last_spr.position.x + last_spr.texture.get_width() - first_spr.position.x - first_spr.texture.get_width() / 2
    # translate all the sprites to center them
    for spr in sprites.get_children():
        spr.position.x -= width / 2


func _grow_left(first_spr: Sprite2D, last_spr: Sprite2D):
    if sprites.get_child_count() == 0:
        return
    var width = last_spr.position.x + last_spr.texture.get_width() / 2 - first_spr.position.x - first_spr.texture.get_width() / 2
    for spr in sprites.get_children():
